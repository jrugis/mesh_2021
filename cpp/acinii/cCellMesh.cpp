...
void cCellMesh::read_mesh_file(std::string mesh_name)
{
  ...
  while (getline(mesh_file, line)) {
   	...
   	if (tokens[0]=="$Nodes" ) break;
  }
  getline(mesh_file, line);
  boost::split(tokens, line, boost::is_any_of(" "), boost::token_compress_on);
  vertices_count = atoi(tokens[0].c_str());
  // get vertices for this cell
  vertices.resize(vertices_count, Eigen::NoChange);
  for(int i=0; i<vertices_count; i++){
  	getline(mesh_file, line);
  	boost::split(tokens, line, boost::is_any_of(" "), boost::token_compress_on);
  	for(int j=0; j<3; j++) vertices(i,j) = std::stod(tokens[j+1]);
  }
  // get the elements count
  while (getline(mesh_file, line)) {
   	boost::split(tokens, line, boost::is_any_of(" "), boost::token_compress_on);
   	if (tokens[0]=="$Elements" ) break;
  }
  getline(mesh_file, line);
  boost::split(tokens, line, boost::is_any_of(" "), boost::token_compress_on);
  int elements_count = atoi(tokens[0].c_str());
  surface_triangles_count = 0;
  // get faces for this cell
  surface_triangles.resize(elements_count, Eigen::NoChange); // overkill for now
  for(int i=0; i<elements_count; i++){
  	getline(mesh_file, line);
  	boost::split(tokens, line, boost::is_any_of(" "), boost::token_compress_on);
    if(tokens[1] != "2") break;
  	for(int j=0; j<3; j++) surface_triangles(i,j) = std::stoi(tokens[j+5])-1;
    surface_triangles_count++;
  }
  surface_triangles.conservativeResize(surface_triangles_count, Eigen::NoChange); // actual size
  // get tetrahedra for this cell
  tetrahedrons_count = elements_count - surface_triangles_count;
  tetrahedrons.resize(tetrahedrons_count, Eigen::NoChange);
  for(int i=0; i<tetrahedrons_count; i++){
  	boost::split(tokens, line, boost::is_any_of(" "), boost::token_compress_on);
  	for(int j=0; j<4; j++) tetrahedrons(i,j) = std::stoi(tokens[j+5])-1;
  	getline(mesh_file, line); // this here because we already had the first line
  }
  mesh_file.close();
}
...
void cCellMesh::write_mesh_file(std::string mesh_name)
{
  ...
  mesh_file << "property float dist_from_lumen\n";
  ...
  mesh_file << "element tetrahedrons " << tetrahedrons_count << "\n";
  mesh_file << "property list uchar uint tetrahedron_indices\n";
  ...
  for(int i=0; i<surface_triangles_count; i++){
	...
	mesh_file << tri_types(i) << "\n";
  }
  for(int i=0; i<tetrahedrons_count; i++){
  	mesh_file << "4 ";
  	mesh_file << tetrahedrons(i,0) << " ";
  	mesh_file << tetrahedrons(i,1) << " ";
  	mesh_file << tetrahedrons(i,2) << " ";
	mesh_file << tetrahedrons(i,3) << "\n";
...
void cCellMesh::calc_nd(cDuctTree* dtree) // triangle to duct measurements 
{
  ...
  // per surface triangle calculations
  ...
  for (int n = 0; n < surface_triangles_count; n++){
    ...
    else { tri_types(n) = BASAL; nbasal++; }
  }
  // per vertex calculations
  v_dnl.resize(vertices_count, Eigen::NoChange);       // distance to the nearest duct segment
  for (int n = 0; n < vertices_count; n++){
  	Vector3d v1 = vertices.row(n);
	v_dnl(n) = dtree->get_dnl(v1);
  }
