...
int main(int argc, char** args)
{
  ...
  // read the duct tree file   
  cDuctTree* dtree = new cDuctTree("./meshes/_lumen_a1.ply");              
  ...
  for (const auto &file : std::filesystem::directory_iterator("./meshes")){
	...
	if(fpath.compare(16,3,"msh") != 0) continue;
	std::cout << fpath << std::endl;
	cCellMesh* cmesh = new cCellMesh(fpath);    // convert a cell mesh file
	cmesh->calc_nd(dtree);
	cmesh->write_mesh_file(fpath.erase(16,3)+"ply");
    delete cmesh;
  }
  delete dtree;
